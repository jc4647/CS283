...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1) 
    {
        ...
        if (strstr(rsp_buff, "command not found") != NULL || strstr(rsp_buff, "No such file or directory") != NULL) {
            printf("Error: Command not found\n");
        }
        else if (strchr(cmd_buff, '$') != NULL || strchr(cmd_buff, '?') != NULL) {
            printf("Warning: Special characters such as '$' or '?' might not work as expected.\n");
